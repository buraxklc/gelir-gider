...
function updateRecurringTransaction($id, $userId, $data) {
    global $pdo;
    ...
    if ($data['is_active']) {
        ...
        // İşlem aktif değilse mevcut değeri koru
        $stmt = $pdo->prepare("SELECT next_occurrence FROM recurring_transactions WHERE id = ? AND user_id = ?");
        $stmt->execute([$id, $userId]);
        $result = $stmt->fetch();
        $nextOccurrence = $result['next_occurrence'] ?? $data['start_date'];
    }
    
    $stmt = $pdo->prepare("
        UPDATE recurring_transactions 
        SET category_id = ?, bank_account_id = ?, type = ?, amount = ?, description = ?,
            frequency = ?, frequency_interval = ?, day_of_week = ?, day_of_month = ?,
            start_date = ?, end_date = ?, next_occurrence = ?, 
            notification_days = ?, is_active = ?
        WHERE id = ? AND user_id = ?
    ");
    
    return $stmt->execute([
        $data['category_id'],
        $data['bank_account_id'], // Artık zorunlu
        $data['type'],
        $data['amount'],
        $data['description'] ?? '',
        $data['frequency'],
